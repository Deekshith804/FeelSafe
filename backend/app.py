...
if __name__ == "__main__":
    ...
    host  = os.getenv("FLASK_HOST",  "0.0.0.0")      # listen on all interfaces (Wi-Fi accessible)
    port  = int(os.getenv("FLASK_PORT",  "5000"))
    debug = os.getenv("FLASK_DEBUG", "false").lower() == "true"

    print(f"[FeelSafe] Backend running on http://{host}:{port}")
    print(f"[FeelSafe] Debug mode     -> {'ON' if debug else 'OFF'}")
