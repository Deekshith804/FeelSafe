...
import { startShakeListener, stopShakeListener } from '../mobile/ShakeSOSController';
...
export default function Emergency() {
  ...
  useEffect(() => {
    ...
    // Load preference
    if (isMobileApp()) {
      Preferences.get({ key: 'shake_sos_enabled' }).then(({ value }) => {
        const isEnabled = value === 'true';
        setShakeSosEnabled(isEnabled);
        if (isEnabled) startShakeListener();
      });
    }
  ...
  const handleToggleShakeSOS = async () => {
    const newState = !shakeSosEnabled;
    setShakeSosEnabled(newState);
    await Preferences.set({ key: 'shake_sos_enabled', value: newState ? 'true' : 'false' });
    if (newState) startShakeListener();
    else stopShakeListener();
  };
    ...
    <div className="min-h-screen relative overflow-hidden flex flex-col items-center p-6 md:p-12">
      ...
      <div className="z-10 w-full max-w-5xl grid grid-cols-1 md:grid-cols-2 gap-10 mt-6">
        ...
        <div className="flex flex-col items-center gap-6">
          ...
          {/* Shake-to-SOS Toggle (Mobile Only) */}
          {isMobileApp() && (
            <div className="w-full max-w-sm glass p-4 rounded-2xl flex items-center justify-between border border-gray-800">
              <div>
                <h3 className="font-bold text-sm">Shake-to-SOS</h3>
                <p className="text-xs text-gray-400">Shake phone 3 times to trigger emergency</p>
              </div>
              <button 
                onClick={handleToggleShakeSOS}
                className={clsx('relative inline-flex h-6 w-11 items-center rounded-full transition-colors', shakeSosEnabled ? 'bg-[#FF3B5C]' : 'bg-gray-600')}
              >
                <span className={clsx('inline-block h-4 w-4 transform rounded-full bg-white transition-transform', shakeSosEnabled ? 'translate-x-6' : 'translate-x-1')} />
              </button>
            </div>
          )}
